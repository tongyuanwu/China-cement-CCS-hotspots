...
China_county_capacity_POP = pd.read_csv('../outputs/China_county_capacity_POP.csv',)
China_county_capacity_GDP = pd.read_csv('../outputs/China_county_capacity_GDP.csv', )
China_county_capacity_combined = pd.read_csv('../outputs/China_county_capacity_combined.csv', )


for t in range(2020, 2055, 5):
    China_counties[f'capacities_{t}_POP'] = China_county_capacity_POP[f'capacities_{t}_POP']
    China_counties[f'capacities_{t}_GDP'] = China_county_capacity_GDP[f'capacities_{t}_GDP']
    China_counties[f'capacities_{t}_combined'] = China_county_capacity_combined[f'capacities_{t}_combined']
...
for i in range(1, 10):
    ...
    China_provinces.plot(color='none', edgecolor='k', lw=0.2, zorder=7, ax=axx)
    ...
    if i in [1, 4, 7]:
        ...
        bounds_1 = [0, 2e3, 4e3, 6e3, 8e3, 10e3, 15e3, 27e3]
        norm_1 = mcolors.BoundaryNorm(bounds_1, plt.cm.Blues.N) 
        China_counties.plot(column=f'capacities_{years[i-1]}_{scenarios[i-1]}', norm=norm_1, cmap=cmap_1, zorder=6, lw=0.1, edgecolor='grey', ax=axx)
        ...
        China_counties.plot(column=f'diff_capacities_{years[i-1]}_{scenarios[i-1]}', norm=norm_2, cmap=cmap_2, zorder=6, lw=0.1, edgecolor='grey', ax=axx)
    ...
    if i == 7:
        ...
        sm.set_array([])
        cbar = plt.colorbar(sm, cax=cax1, orientation='horizontal', label='capacity (Mt clinker/year)')
        # cbar.ax.set_xticklabels([0, '', 1, '', 2, '', 3, '', 4, '', 5])
        cbar.ax.set_xticklabels([0, 2, 4, 6, 8, 10, 15, 27])
...
plt.show()
fig.savefig('../figures/Fig_6.jpg', dpi=600, bbox_inches='tight')